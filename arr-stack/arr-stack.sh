...
log "Homelab Arr Stack guided installer"

HOST_TYPE="$(choose_option "Where are you installing the containers?" \
  "Ubuntu/Debian Linux server" \
  "Synology or UGREEN NAS Docker host" \
  "Generic Docker host with Docker already installed")"
...
case "$HOST_TYPE" in
  "Ubuntu/Debian Linux server")
    ...
    ;;
  "Synology or UGREEN NAS Docker host")
    printf 'This mode assumes Docker/Container Manager is already available on the NAS.\n'
    ;;
  "Generic Docker host with Docker already installed")
    printf 'This mode skips package and Docker installation.\n'
    ;;
esac
...
if [[ "$INSTALL_PACKAGES" == "true" ]]; then
  ...
  die "Docker was not found. Install Docker first or choose the Ubuntu/Debian setup option."
else
  require_docker_compose
fi
...
if [[ "$SETUP_NFS" == "true" ]]; then
  ...
  if [[ "$HOST_TYPE" == "Synology or UGREEN NAS Docker host" ]]; then
