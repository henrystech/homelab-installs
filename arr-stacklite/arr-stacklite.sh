...
REPO_RAW_URL="${REPO_RAW_URL:-https://raw.githubusercontent.com/henrystech/homelab-installs/main/arr-stacklite}"
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    die "Please run as root: sudo ./arr-stacklite.sh"
  fi
...
prepare_directories() {
  ...
  chown -R "$PUID:$PGID" \
    ...
    "$DOCKERSTORAGEDIR" || true
}

require_root

log "Ubuntu Arr Stack Lite guided installer"
